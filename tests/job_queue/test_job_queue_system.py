...
def test_rqmanager_enqueue_pipeline(monkeypatch):
    ...
    with mock.patch.object(
        manager, "_enqueue_job", return_value="jobid123"
    ) as mock_enqueue:
...
def test_rqmanager_schedule_pipeline(monkeypatch):
    manager = RQManager(config={"redis_url": "redis://localhost:6379/0"})
    with mock.patch.object(
        manager, "_schedule_job", return_value="jobid456"
    ) as mock_sched:
        job_id = manager.schedule_pipeline(
            "pipeline_name", args={"y": 2}, eta=1234567890
        )
...
def test_rqmanager_job_management(monkeypatch):
    ...
    # get_job
    with mock.patch.object(
        manager, "_get_job", return_value=JobInfo(id="jid", status="finished")
    ):
    ...
    # list_jobs
    with mock.patch.object(
        manager, "_list_jobs", return_value=[JobInfo(id="jid", status="finished")]
    ):
...
def test_rqmanager_queue_and_worker_management(monkeypatch):
    ...
    with mock.patch.object(
        manager, "list_workers", return_value=["worker1", "worker2"]
    ):
...
class BaseTestJobQueueManager:
    ...
    def test_enqueue_and_get_job(self, backend):
        ...
        with mock.patch.object(
            backend, "get_job", return_value=JobInfo(id="jobid", status="queued")
        ):
