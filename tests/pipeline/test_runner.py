...
from flowerpower.cfg.pipeline import PipelineRunConfig, PipelineAdapterConfig, ExecutorConfig, WithAdapterConfig
...
class TestPipelineRunner(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_pipeline_cfg = PipelineConfig(
            ...
            run=PipelineRunConfig( # Assuming default run config is okay, or mock as needed
                inputs={},
                final_vars=[],
                executor=ExecutorConfig(type="synchronous"), # Default to synchronous for basic tests
    ...
    def test_pipeline_execution_with_threadpool_executor(self):
        ...
        results = self.runner.run(final_vars=['output_value'], executor_cfg=executor_config)
            ...
            with self.subTest(adapter_type=adapter_type):
                ...
                try:
                    results = self.runner.run(
                        with_adapter_cfg=adapter_config,
                        final_vars=['output_value']
                    )
                    self.assertEqual(results['output_value'], 25)
                    ...
                    self.fail(f"Pipeline run failed with adapter {adapter_type} enabled: {e}")
    ...
    def test_retry_mechanism(self):
        ...
        with self.assertRaises(ValueError): # Expecting a ValueError after retries are exhausted
            self.runner.run(
