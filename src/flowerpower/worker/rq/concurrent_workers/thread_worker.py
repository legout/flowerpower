...
class ThreadWorker(worker.Worker):
    ...
    """
    def __init__(self, queues, name=None, max_threads=None,
        ...
        self.max_threads = max_threads if max_threads is not None else (os.cpu_count() or 1) * 5
        self._executor = None
        self._futures = set()
        self.log = logger
        ...
        try:
            while True:
                ...
                if len(self._futures) >= self.max_threads:
                        ...
                        self._futures, timeout=1.0, return_when=concurrent.futures.FIRST_COMPLETED
                    )
                    for future in done:
                        try:
                            future.result()
                        except Exception as e:
                            self.log.error(f"Error in completed job: {e}", exc_info=True)
                    continue
