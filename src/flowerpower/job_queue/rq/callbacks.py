...
class CallbackRegistry:
    ...
    @classmethod
    def resolve_callback(cls, callback_info: Union[str, Callable, None]) -> Union[Callable, None]:
        """
        ...
        if isinstance(callback_info, str):
            ...
            try:
                ...
                if callable(func):
                    ...
                    logger.warning(f"Resolved '{callback_info}' but it is not callable.")
                    return None
            except (ImportError, AttributeError, ValueError) as e:
                logger.error(f"Could not resolve callback string '{callback_info}': {e}")
                return None
        logger.warning(f"Invalid callback info type: {type(callback_info)}. Must be str, Callable, or None.")
        return None
