...
def _execute_callback(callback_info: Any, context_exception: Exception = None):
        ...
        if context_exception:
            try:
                ...
                if len(sig.parameters) == 1:  # Assumes it takes one positional argument
                    ...
                    if sig.parameters[first_param_name].kind in [inspect.Parameter.POSITIONAL_OR_KEYWORD, inspect.Parameter.POSITIONAL_ONLY]:
                        cb_args = (context_exception,)
                elif 'exception' in sig.parameters: # Or if it explicitly takes 'exception' kwarg
                    cb_kwargs['exception'] = context_exception
            except (ValueError, TypeError): # Some callables might not be inspectable
                logger.debug(f"Could not inspect signature for simple callback {getattr(callback_fn, '__name__', str(callback_fn))}. Exception not passed automatically.")
    ...
    if callback_fn:
        try:
            ...
            logger.error(f"Error executing callback {callback_name_err}: {cb_e}", exc_info=True)
    else:
        logger.warning(f"Invalid callback_info format or non-callable provided: {callback_info}")
            ...
            try:
                ...
                if success_flag:
                    ...
                    if on_failure:
                        ...
                        _execute_callback(on_failure, context_exception=caught_exception)
        return wrapper
    return decorator