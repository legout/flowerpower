...
class RQBackendConfig(JobQueueBackendConfig):
    ...
    def update_from_settings(self):
        if self.host == BACKEND_PROPERTIES["redis"]["default_host"]:
            self.host = settings.RQ_BACKEND_HOST
        if self.port == BACKEND_PROPERTIES["redis"]["default_port"]:
            self.port = settings.RQ_BACKEND_PORT
        if self.database == BACKEND_PROPERTIES["redis"]["default_database"]:
            self.database = settings.RQ_BACKEND_DB
        if self.username == BACKEND_PROPERTIES["redis"]["default_username"]:
            self.username = settings.RQ_BACKEND_USERNAME
        if self.password == BACKEND_PROPERTIES["redis"]["default_password"]:
