...
def model_evaluation(
    ...
    for metric in scoring_metrics:
        if metric in ["accuracy", "precision", "recall", "f1", "roc_auc"]:
            ...
            logger.info(
                f"CV {metric}: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})"
            )
    ...
    if y_pred_proba is not None:
        evaluation_results["test_performance"]["roc_auc"] = roc_auc_score(
            y_test, y_pred_proba
        )
    ...
    logger.info(
        f"Test accuracy: {evaluation_results['test_performance']['accuracy']:.4f}"
    )
        ...
        try:
            ...
            if "preprocessor_file" in artifacts:
                latest_preprocessor.symlink_to(
                    Path(artifacts["preprocessor_file"]).name
                )
