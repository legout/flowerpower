...
def filtered_sales_data(
    ...
    logger.info(
        f"Filtered to {len(filtered)} sales records for {report_frequency} report"
    )
    return filtered
...
def sales_kpis(filtered_sales_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Daily breakdown
    daily_sales = filtered_sales_data.groupby(filtered_sales_data["sale_date"].dt.date)[
        "revenue"
    ].sum()

    # Product performance
    product_sales = (
        filtered_sales_data.groupby("product_name")
    ...
    logger.info(
        f"Calculated sales KPIs: Revenue=${total_revenue:,.2f}, Orders={total_orders}"
    )
    return kpis


def inventory_kpis(
    inventory_data: pd.DataFrame, low_inventory_threshold: int
) -> Dict[str, Any]:
    ...
    kpis = {
        ...
        "low_stock_items": low_stock_items[["product_name", "quantity"]].to_dict(
            "records"
        ),
    ...
    logger.info(
        f"Calculated inventory KPIs: {total_products} products, {len(low_stock_items)} low stock"
    )
    return kpis


def customer_kpis(
    customer_data: pd.DataFrame, current_date: str, customer_retention_target: float
) -> Dict[str, Any]:
    ...
    active_customers = customer_data[customer_data["last_purchase"] >= active_threshold]

    # Retention calculation
    retention_rate = (
        len(active_customers) / len(customer_data) if len(customer_data) > 0 else 0
    )
    ...
    new_customers = customer_data[customer_data["signup_date"] >= month_start]
    ...
    logger.info(
        f"Calculated customer KPIs: {len(customer_data)} total, {retention_rate:.2%} retention"
    )
    return kpis


def sales_charts(
    sales_kpis: Dict[str, Any], include_charts: bool, chart_style: str
) -> Dict[str, Any]:
    ...
    if top_products:
        ...
        fig_products = go.Figure(
            data=[go.Bar(x=products[:5], y=revenues[:5], marker_color="#ff7f0e")]
        )
