...
def run_batch_processing():
    ...
    print("   flowerpower job-queue start-worker --queue-names small_batches,medium_batches,large_batches")
...
def run_notifications():
    ...
    notification_configs = [
        {"recipient": "admin@company.com", "message": "System backup completed", "channel": "email"},
        {"recipient": "+1234567890", "message": "Alert: High CPU usage detected", "channel": "sms"},
        {"recipient": "#alerts", "message": "Daily report is ready", "channel": "slack"},
        {"recipient": "user@company.com", "message": "Your report has been generated", "channel": "email"}
    ]
...
def run_data_validation():
    ...
    for i in range(20):
        item = {
            ...
            "email": f"user{i}@example.com" if i % 5 != 0 else "invalid_email",  # Some invalid emails
            "category": random.choice(["A", "B", "C", "INVALID"]),  # Some invalid categories
        }
    ...
    print(f"📊 Validating {len(sample_data)} items against {len(validation_rules)} rules")
...
def run_mixed_workload():
    ...
    from tasks.data_processing import (
        simple_calculation, process_batch_data, generate_report,
        send_notification, cleanup_old_files
    )
    ...
    jobs_to_enqueue = [
        # Quick calculations
        {"func": simple_calculation, "queue": "quick", "args": {"x": 15, "y": 3, "operation": "multiply"}},
        {"func": simple_calculation, "queue": "quick", "args": {"x": 100, "y": 7, "operation": "divide"}},
        
        # Reports
        {"func": generate_report, "queue": "reports", "args": {"data_source": "mixed_data.csv", "report_type": "summary"}},
        
        # Notifications
        {"func": send_notification, "queue": "notifications", "args": {"recipient": "ops@company.com", "message": "Mixed workload test", "channel": "email"}},
        
        # Maintenance
        {"func": cleanup_old_files, "queue": "maintenance", "args": {"directory": "/tmp/test", "days_old": 1}},
        
        # Batch processing
        {"func": process_batch_data, "queue": "batch", "args": {
    ...
    for job_config in jobs_to_enqueue:
        ...
        all_jobs.append(job)
        print(f"✅ Enqueued {job_config['func'].__name__} to {job_config['queue']}: {job.id}")
    
    print(f"\n📋 Total mixed jobs enqueued: {len(all_jobs)}")
    print("🚀 To process all job types, start workers:")
    print("   flowerpower job-queue start-worker --queue-names quick,reports,notifications,maintenance,batch")
    ...
    try:
        ...
        if workers:
            ...
            for worker in workers:
                print(f"   🔧 {worker.get('name', 'Unknown')}: {worker.get('state', 'Unknown')}")
        else:
