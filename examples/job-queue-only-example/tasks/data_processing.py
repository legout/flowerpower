...
def process_batch_data(data_list: List[Dict[str, Any]], batch_size: int = 10) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(f"Processing batch of {len(data_list)} items in batches of {batch_size}")
    ...
    return {
        ...
        "avg_items_per_second": len(processed_items) / total_duration if total_duration > 0 else 0
    }
...
def long_running_computation(iterations: int = 100, delay_ms: int = 100) -> Dict[str, Any]:
    """
...
def data_validation_task(data: List[Dict[str, Any]], validation_rules: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(f"Validating {len(data)} data items against {len(validation_rules)} rules")
        ...
        for field, rules in validation_rules.items():
            ...
            if "type" in rules:
                ...
                if expected_type == "int" and not isinstance(value, int):
                    item_valid = False
                    item_errors.append(f"Field {field} must be integer, got {type(value).__name__}")
                elif expected_type == "str" and not isinstance(value, str):
                    item_valid = False
                    item_errors.append(f"Field {field} must be string, got {type(value).__name__}")
            
            # Range validation for numbers
            if isinstance(value, (int, float)):
                if "min" in rules and value < rules["min"]:
                    item_valid = False
                    item_errors.append(f"Field {field} value {value} below minimum {rules['min']}")
                if "max" in rules and value > rules["max"]:
                    item_valid = False
                    item_errors.append(f"Field {field} value {value} above maximum {rules['max']}")
            
            # Length validation for strings
            if isinstance(value, str):
                if "min_length" in rules and len(value) < rules["min_length"]:
                    item_valid = False
                    item_errors.append(f"Field {field} too short: {len(value)} < {rules['min_length']}")
                if "max_length" in rules and len(value) > rules["max_length"]:
                    item_valid = False
                    item_errors.append(f"Field {field} too long: {len(value)} > {rules['max_length']}")
