...
def filtered_chunks(
    ...
    logger.info(f"Filtered to {len(filtered)} chunks from {len(text_chunks)} original chunks")
    return filtered
...
def word_statistics(
    ...
    logger.info(f"Calculated word statistics: {len(all_words)} total words, {len(word_freq)} unique")
    return statistics
...
def sentiment_analysis(
    ...
    positive_words = [
        ...
        'success', 'win', 'best', 'love', 'like', 'happy', 'pleased', 'satisfied',
        'brilliant', 'outstanding', 'superb', 'magnificent', 'perfect', 'beautiful'
    ]
    
    negative_words = [
        'bad', 'terrible', 'awful', 'horrible', 'negative', 'failure', 'lose', 
        'worst', 'hate', 'dislike', 'sad', 'angry', 'disappointed', 'frustrated',
        'ugly', 'disgusting', 'pathetic', 'miserable', 'dreadful', 'appalling'
    ]
    ...
    analysis = {
        ...
        'sentiment_score': (overall_positive - overall_negative) / max(1, overall_positive + overall_negative),
    ...
    if include_statistics:
        results['processing_statistics'] = {
            'analysis_modules_used': [
                key for key in ['word_analysis', 'sentence_analysis', 'character_analysis', 
                              'keyword_analysis', 'sentiment_analysis'] 
                if key in results
            ],
